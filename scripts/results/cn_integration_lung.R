...
pdf('../results/scaled/limma/integration/CNvariation_sample_wise_correlation_lung_scaled.pdf')
ggplot(merged_long, aes(x = variable, y=value, fill = variable)) + geom_boxplot() + 
  stat_compare_means(comparisons = comparisons, label = 'p.signif', vjust=0.75, label.x = 5, label.y = c(0.12, 0.13,0.14,0.15,0.16), hide.ns = TRUE, tip.length = 0.02) +
