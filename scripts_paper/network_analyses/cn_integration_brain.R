...
SWEET_SOW <- calculate_SOW('./SWEET/SWEET_brain_no_doubles_no_loops_HN_-1_1scaled_top_25000_edges.csv', offset=FALSE)
...
SWEET <- gene_wise_corr(cn_dat, SWEET_SOW); SWEET$gene <- rownames(SWEET)
...
comparisons <- list(#c('Expression', 'SSN'), c('Expression', 'LIONESS'), c('Expression', 'iENA'), c('Expression', 'CSN'), c('Expression', 'SSPGI'), c('Expression', 'Aggregate'),
  ...
  c('CSN', 'LIONESS'), c('iENA', 'LIONESS'), c('CSN', 'SSN'), c('iENA', 'SSN'), c('CSN', 'SSPGI'), c('CSN', 'SWEET'), c('iENA', 'SWEET'))

 
pdf('../results/Rebuttal/SWEET/CNV_sample_wise_correlation_brain_scaled_withSWEET.pdf')
ggplot(merged_long, aes(x = variable, y=value, fill = variable)) + geom_boxplot() +
  stat_compare_means(comparisons = comparisons, label = 'p.signif', vjust=0.75, label.x = 5, label.y = c(0.12, 0.13,0.14,0.15,0.16, 0.17, 0.18, 0.19, 0.2, 0.21, 0.22, 0.23, 0.24), hide.ns = TRUE, tip.length = 0.02) +
...
# 
# pdf('/home/boris/shared/CNV_brain_noExpr_poster.pdf')
# ggplot(merged_long, aes(x = variable, y=value, fill = variable)) + geom_boxplot() + 
#   stat_compare_means(comparisons = comparisons, label = 'p.signif', vjust=0.75, label.x = 5, label.y = c(0.11, 0.12,0.13,0.14,0.15), tip.length = 0.02) +
#   stat_compare_means(label.x = 4.75, label.y = 0.09) +
