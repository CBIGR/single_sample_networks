...
#write.table(res, file = '../results/Rebuttal/SWEET/proteomics_sample_wise_correlation_brain_stats.txt', sep = '\t', quote = FALSE, row.names = FALSE)
...
comparisons <- list(#c('Expression', 'SSN'), c('Expression', 'LIONESS'), c('Expression', 'iENA'), c('Expression', 'CSN'), c('Expression', 'SSPGI'), c('Expression', 'Aggregate'),
  c('Aggregate', 'SSPGI'), c('Aggregate', 'iENA'), c('Aggregate', 'SWEET'), c('Aggregate', 'LIONESS'), c('Aggregate', 'SSN'),
  c('CSN', 'SWEET'), c('CSN', 'LIONESS'), c('CSN', 'SSN'), c('CSN', 'SSPGI'))



pdf('../results/Rebuttal/SWEET/proteomics_sample_wise_correlation_brain_scaled_withSWEET.pdf')
ggplot(merged_long, aes(x = variable, y=value, fill = variable)) + geom_boxplot() + 
  stat_compare_means(comparisons = comparisons, label = 'p.signif', vjust=0.75, label.x = 5, label.y = c(0.4, 0.42, 0.44, 0.46, 0.48, 0.5, 0.52, 0.54, 0.56),hide.ns = TRUE, tip.length = 0.02) +
  stat_compare_means(label.x = 2.4, label.y=0.42) +
