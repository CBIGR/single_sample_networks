...
overlapWithDrivers <- function(top200list, cancerType, driverlist){
  ...
  } else {samples <- sample_metadata[sample_metadata$lineage_sub_subtype == cancerType, ]}
...
Adeno_drivers <- driver_mutations[driver_mutations$DepMap_ID %in% Adeno_samples$DepMap_ID, ]
Squamous_drivers <- driver_mutations[driver_mutations$DepMap_ID %in% Squamous_samples$DepMap_ID, ]
LargeCell_drivers <- driver_mutations[driver_mutations$DepMap_ID %in% LargeCell_samples$DepMap_ID, ]
NSCLC_drivers <- driver_mutations[driver_mutations$DepMap_ID %in% NSCLC_samples$DepMap_ID, ]

# SCLC_drivers_pass <- driver_mutations[driver_mutations$DepMap_ID %in% SCLC_samples$DepMap_ID, ]
# Adeno_drivers_pass <- driver_mutations[driver_mutations$DepMap_ID %in% Adeno_samples$DepMap_ID, ]
# Squamous_drivers_pass <- driver_mutations[driver_mutations$DepMap_ID %in% Squamous_samples$DepMap_ID, ]
# LargeCell_drivers_pass <- driver_mutations[driver_mutations$DepMap_ID %in% LargeCell_samples$DepMap_ID, ]
# NSCLC_drivers_pass <- driver_mutations[driver_mutations$DepMap_ID %in% NSCLC_samples$DepMap_ID, ]

# SCLC_drivers <- unique(SCLC_mutations$gene_symbol)
# Adeno_drivers <- unique(Adeno_mutations$gene_symbol)
# Squamous_drivers <- unique(Squamous_mutations$gene_symbol)
# LargeCell_drivers <- unique(LargeCell_mutations$gene_symbol)
...
p_values_LargeCell <- data.frame(matrix(nrow = 6, ncol=length(number_of_hubs)))
rownames(p_values_LargeCell) <- c('LIONESS_lung', 'SSN_lung', 'ssPCC_lung', 'CSN_lung', 'SSPGI_lung', 'SWEET_lung'); colnames(p_values_LargeCell) <- number_of_hubs
...
fold_changes_LargeCell <- data.frame(matrix(nrow = 6, ncol=length(number_of_hubs)))
rownames(fold_changes_LargeCell) <- c('LIONESS_lung', 'SSN_lung', 'ssPCC_lung', 'CSN_lung', 'SSPGI_lung', 'SWEET_lung'); colnames(fold_changes_LargeCell) <- number_of_hubs
...
for (j in (1:length(number_of_hubs))){
  ...
  SWEET_lung <- top200nodes(paste0(base,"SWEET/SWEET_lung_no_doubles_no_loops_HN_-1_1scaled_top_25000_edges.csv"), k, offset=FALSE)
  ...
  type <- 'NSCLC_large_cell'
  print('Large cell')
  SSN_LargeCell <- overlapWithDrivers(SSN_lung, type, LargeCell_drivers)
  LIONESS_LargeCell <- overlapWithDrivers(LIONESS_lung, type, LargeCell_drivers)
  SWEET_LargeCell <- overlapWithDrivers(SWEET_lung, type, LargeCell_drivers)
  ssPCC_LargeCell <- overlapWithDrivers(iENA_lung, type, LargeCell_drivers)
  CSN_LargeCell <- overlapWithDrivers(CSN_lung, type, LargeCell_drivers)
  SSPGI_LargeCell <- overlapWithDrivers(SSPGI_lung, type, LargeCell_drivers)
  ...
  p_values_LargeCell['LIONESS_lung', j] <- test_hyper(LIONESS_lung, LargeCell_drivers, LargeCell_samples)$test
  p_values_LargeCell['SSN_lung', j] <- test_hyper(SSN_lung, LargeCell_drivers, LargeCell_samples)$test
  p_values_LargeCell['ssPCC_lung', j] <- test_hyper(iENA_lung, LargeCell_drivers, LargeCell_samples)$test
  p_values_LargeCell['CSN_lung', j] <- test_hyper(CSN_lung, LargeCell_drivers, LargeCell_samples)$test
  p_values_LargeCell['SSPGI_lung', j] <- test_hyper(SSPGI_lung, LargeCell_drivers, LargeCell_samples)$test
  p_values_LargeCell['SWEET_lung', j] <- test_hyper(SWEET_lung, LargeCell_drivers, LargeCell_samples)$test
  ...
  fold_changes_LargeCell['LIONESS_lung', j] <- test_hyper(LIONESS_lung, LargeCell_drivers, LargeCell_samples)$fold_change
  fold_changes_LargeCell['SSN_lung', j] <- test_hyper(SSN_lung, LargeCell_drivers, LargeCell_samples)$fold_change
  fold_changes_LargeCell['ssPCC_lung', j] <- test_hyper(iENA_lung, LargeCell_drivers, LargeCell_samples)$fold_change
  fold_changes_LargeCell['CSN_lung', j] <- test_hyper(CSN_lung, LargeCell_drivers, LargeCell_samples)$fold_change
  fold_changes_LargeCell['SSPGI_lung', j] <- test_hyper(SSPGI_lung, LargeCell_drivers, LargeCell_samples)$fold_change
  fold_changes_LargeCell['SWEET_lung', j] <- test_hyper(SWEET_lung, LargeCell_drivers, LargeCell_samples)$fold_change
  ...
  # write.table(p_values_Adeno, '../results/top200_connected_nodes/lung/p_values_Adeno.txt', row.names = TRUE, sep = '\t', quote = FALSE)
  # write.table(p_values_Squamous, '../results/top200_connected_nodes/lung/p_values_Squamous.txt', row.names = TRUE, sep = '\t', quote = FALSE)
  # 
  # write.table(fold_changes_SCLC, '../results/top200_connected_nodes/lung/fold_changes_SCLC.txt', row.names = TRUE, sep = '\t', quote = FALSE)
  # write.table(fold_changes_Adeno, '../results/top200_connected_nodes/lung/fold_changes_Adeno.txt', row.names = TRUE, sep = '\t', quote = FALSE)
  # write.table(fold_changes_Squamous, '../results/top200_connected_nodes/lung/fold_changes_Squamous.txt', row.names = TRUE, sep = '\t', quote = FALSE)
...
length(Reduce(intersect, SSN_lung))
length(Reduce(intersect, LIONESS_lung))
length(Reduce(intersect, SWEET_lung))
length(Reduce(intersect, iENA_lung))
length(Reduce(intersect, CSN_lung))
length(Reduce(intersect, SSPGI_lung))



save.image('../results/Rebuttal/top200_connected_nodes/results_top200_lung_CellPassports_drivers_withSWEET.RData')
