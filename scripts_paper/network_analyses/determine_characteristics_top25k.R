...
SWEET_lung <- fread(paste0(base, "SWEET/SWEET_lung_no_doubles_no_loops_HN_-1_1scaled_top_25000_edges.csv"), header=TRUE, data.table=FALSE, fill=TRUE)
SWEET_lung_genes <- unique(as.vector(as.matrix(SWEET_lung[,c(1,2)])))
SWEET_brain <- fread(paste0(base, "SWEET/SWEET_brain_no_doubles_no_loops_HN_-1_1scaled_top_25000_edges.csv"), header=TRUE, data.table=FALSE, fill=TRUE)
SWEET_brain_genes <- unique(as.vector(as.matrix(SWEET_brain[,c(1,2)])))


single_sample_networks <- list(paste0(base, "SSN/JDS_SSN_2_75_lung_no_doubles_no_loops_HumanNet_top_25000_edges.csv"),
                               ...
                               paste0(base, "LIONESS/JDS_LIONESS_PCC_2_75_brain_no_doubles_no_loops_HumanNet_top_25000_edges.csv"),
                               paste0(base, "SWEET/SWEET_lung_no_doubles_no_loops_HN_top_25000_edges.csv"),
                               paste0(base, "SWEET/SWEET_brain_no_doubles_no_loops_HN_top_25000_edges.csv"),
...
output_files <- list("SSN_lung_HumanNet_top25k.txt", 
                     "SSN_brain_HumanNet_top25k.txt",
                     "LIONESS_lung_HumanNet_top25k.txt", 
                     "LIONESS_brain_HumanNet_top25k.txt",
                     "SWEET_lung_HumanNet_top25k.txt",
                     "SWEET_brain_HumanNet_top25k.txt",
                     "ssPCC_lung_HumanNet_top25k.txt",
                     "ssPCC_brain_HumanNet_top25k.txt",
                     "CSN_brain_HumanNet_top25k.txt",
                     "CSN_brain_HumanNet_top25k.txt",
                     "SSPGI_brain_HumanNet_top25k.txt",
                     "SSPGI_lung_HumanNet_top25k.txt")

offsets <- list(TRUE,TRUE, TRUE, TRUE, FALSE,FALSE,TRUE,TRUE,FALSE,FALSE,TRUE,TRUE)
library(parallel)
library(MASS)
...
colnames(results) <- c('SSN_lung', 'SSN_brain', 'lioness_lung', 'lioness_brain', 'SWEET_lung', 'SWEET_brain', 'iENA_lung', 'iENA_brain','CSN_brain', 'CSN_lung', 'SSPGI_brain', 'SSPGI_lung')

res_table <- data.frame(matrix(nrow = 6, ncol = 12))
rownames(res_table) <- row.names(results); colnames(res_table) <-c('SSN_lung', 'SSN_brain', 'lioness_lung', 'lioness_brain', 'SWEET_lung', 'SWEET_brain', 'iENA_lung', 'iENA_brain','CSN_brain', 'CSN_lung', 'SSPGI_brain', 'SSPGI_lung')
...
write.table(res_table, '../results/Rebuttal/SWEET/network_characteristics_overview_table.txt', quote=FALSE, sep='\t')
