...
comparisons <- list(#c('Expression', 'SSN'), c('Expression', 'LIONESS'), c('Expression', 'iENA'), c('Expression', 'CSN'), c('Expression', 'SSPGI'), c('Expression', 'Aggregate'),
  ...
  c('CSN', 'LIONESS'), c('iENA', 'LIONESS'), c('CSN', 'SSN'), c('iENA', 'SSN'), c('CSN', 'SSPGI'), c('CSN', 'SWEET'))


pdf('../results/Rebuttal/SWEET/CNV_sample_wise_correlation_lung_scaled_withSWEET.pdf')
ggplot(merged_long, aes(x = variable, y=value, fill = variable)) + geom_boxplot() + 
  stat_compare_means(comparisons = comparisons, label = 'p.signif', vjust=0.75, label.x = 5, label.y = c(0.12, 0.13,0.14,0.15,0.16, 0.17, 0.18, 0.19, 0.2, 0.21, 0.22, 0.23), hide.ns = TRUE, tip.length = 0.02) +
  stat_compare_means(label.x = 5.65, label.y=0.11) +
