...
LIONESS <- gene_wise_corr(RNAseq_coding, LIONESS_SOW); LIONESS$gene <- rownames(LIONESS)
SSN <- gene_wise_corr(RNAseq_coding, SSN_SOW); SSN$gene <- rownames(SSN)
CSN <- gene_wise_corr(RNAseq_coding, CSN_SOW); CSN$gene <- rownames(CSN)
iENA <- gene_wise_corr(RNAseq_coding, iENA_SOW); iENA$gene <- rownames(iENA)
SSPGI <- gene_wise_corr(RNAseq_coding, SSPGI_SOW); SSPGI$gene <- rownames(SSPGI)
...
pdf('../results/scaled/limma/integration/expression_sample_wise_correlation_lung_scaled.pdf')
ggplot(merged_long, aes(x = variable, y=value, fill = variable)) + geom_boxplot() + 
  ...
  ggtitle('Mean absolute correlation coefficient\nwith expression') + scale_fill_manual(values=cbbPalette) +
